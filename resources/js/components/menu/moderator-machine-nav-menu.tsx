...
export default function ModeratorMachineNavMenu({ onSearch, onPageChange, links, initialPage = 1 }: Props) {
    ...
    const submitSearch = (e: FormEvent) => {
        ...
        else console.log('Search machines:', q);
    };
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(() => {
            if (typeof onSearch === 'function') onSearch(q);
            else console.log('Auto search machines:', q);
        }, 350);
    ...
    return (
        <nav className="w-full bg-white border p-2 sm:p-3 flex items-center gap-4 justify-start mx-4 sm:mx-6">
            <form onSubmit={submitSearch} className="flex items-center gap-2 w-72 mr-6 sm:mr-8">
                <input
                    aria-label="Szukaj maszyny"
                    placeholder="Szukaj barcode / nazwy / modelu"
                    value={query}
